...
이 문서는 현재 SCIE/IEEE Access 원고로 전환하기 전에 남은 작업을 구분하기 위한 체크리스트이다. 지민이 직접 확인해야 하는 연구자 검토 항목과, 문서/표/그림 작성 항목을 분리한다.
...
| G4 자동 단계 분류 구현 및 평가 | 완료 | `29_stage_classifier_results.md` |
| G1/G2/G3/G4 검색 성능 평가 | 완료 | `15_g1_g2_g3_g4_results.md` |
| G4 개선/실패 사례 분석 | 완료 | `16_g4_case_analysis.md` |
| 응답 품질 자동 1차 평가 | 완료 | `22_response_quality_eval_results.md` |
...
| 우선순위 | 항목 | 이유 | 파일 |
|---:|---|---|---|
| 1 | 연구자 검토 체크리스트 확인 | 자동 평가를 연구자 검토로 보완하기 위해 필요 | `excel/31_researcher_review_checklist.xlsx` |
| 2 | 대표 사례 이미지 육안 확인 | 논문에 사례 그림을 넣을 때 실제 이미지가 질문과 맞는지 확인 필요 | `data/processed/final_refined_data/` |
| 3 | 교수님에게 보여줄 최종 보고 파일 선택 | 보고 분량 조절 필요 | `SCIE용/산출물/` |
...
| 피해야 할 표현 | 이유 | 대체 표현 |
|---|---|---|
| 연구자 평가를 완료했다 | 아직 지민의 수동 검토가 남아 있음 | AI-assisted automatic evaluation with a researcher review checklist |
...
1. 지민이 `31_researcher_review_checklist.xlsx`에서 우선순위 상 항목부터 확인한다.
2. 그동안 영문 초안 변환 또는 Figure 1/4/5 제작을 진행한다.
3. 연구자 검토 결과가 들어오면 `22_response_quality_eval_results.md`와 `19_paper_draft.md`를 최종 반영한다.
4. 참고문헌을 IEEE 양식으로 정리한다.
