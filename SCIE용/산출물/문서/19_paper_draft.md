...
실험 데이터는 두산로보틱스 DART-Platform 매뉴얼을 기반으로 구성하였다[30]. 해당 매뉴얼과 수록 시각 자료는 두산로보틱스의 확인에 따라 본 연구에 활용하였으며, 원 출처는 참고문헌 [30]에 명시하였다. 총 70개의 평가 질의를 작성하고, 각 질의에 정답 텍스트, 정답 이미지 및 실습 단계 라벨을 부여하였다. 질의 범주는 설치, 전원, 티치 펜던트 조작, 직접 교시, 좌표 설정, 안전 설정, I/O 배선 및 시스템 관리를 포함한다.
