...
G3에서는 이미지 검색이 추가되었고, BBox를 후보 필터로 제한한 뒤 SigLIP으로 매핑 순위를 정하도록 개선한 결과 Image Recall@1은 45.7%, Image MRR은 0.572로 나타났다. 이전 파일럿보다 상위 순위 품질은 좋아졌지만, 정답 이미지를 1위에 배치한 비율은 여전히 절반 미만이다.

G4에서는 질문에서 실습 단계를 추정한 뒤, 해당 단계의 page 범위, section heading, keyword 정보를 기반으로 re-ranking을 수행하였다. 그 결과 G3 대비 Image Recall@5는 71.4%에서 77.1%로, Image Recall@10은 80.0%에서 87.1%로, Image MRR은 0.572에서 0.620으로 개선되었다. 다만 Image Recall@1은 48.6%로 절반 미만이므로, G4는 이미지 검색 문제를 해결한 방식이 아니라 관련 이미지 후보를 Top-5 또는 Top-10 안으로 더 잘 끌어올린 방식으로 해석한다.
...
- 다만 Image Recall@1은 48.6%로 절반 미만이어서, 이미지 검색 문제를 완전히 해결했다고 주장하지 않는다.
