...
- G4는 질문 기반 단계 추정을 사용해 실제 앱 조건에서도 G3 대비 Image Recall@5, Image Recall@10, Image MRR을 개선하는지 확인한다.
- 단, Image Recall@1이 낮게 유지될 수 있으므로 G4의 성과는 이미지 검색 문제의 해결이 아니라 관련 이미지 후보 순위 개선으로 해석한다.
