...
본 연구는 협동 로봇 실습 매뉴얼을 대상으로, 학습자의 실습 질의에 대해 텍스트 설명과 이미지/도식 자료를 함께 검색하고 이를 기반으로 안내 응답을 생성하는 상황 인지형 멀티모달 RAG 구조를 제안한다. 또한 외부 API 없이 로컬 벡터 DB와 4-bit 양자화 LLM을 활용하여 오프라인 및 8GB RAM급 제한 환경에서 활용 가능한 구조를 목표로 한다.
...
현재 논문용 기준 결과는 단계 추정 기반 G4 결과를 기준으로 한다. 정답 실습 단계가 주어진 oracle-stage 결과는 실제 앱 사용 조건이 아니므로 본문 메인 비교에는 사용하지 않고, 필요한 경우 부록 또는 추가 분석에서 상한 성능으로만 언급한다. `09_stage_context_map` 및 `10_g4_results`는 초기 예비 실험 성격이므로 최종 논문 결과로 사용하지 않는다.
...
G4에서는 질문에서 실습 단계를 추정한 뒤, 해당 단계의 page 범위, section heading, keyword 정보를 기반으로 re-ranking을 수행하였다. 그 결과 G3 대비 Image Recall@5는 70.0%에서 75.7%로, Image Recall@10은 75.7%에서 87.1%로, Image MRR은 0.485에서 0.539로 개선되었다.
...
이 평가는 rubric 기반 1차 평가이며, 이후 G4 결과 중 오류 및 애매 사례 중심의 54개 우선 검토 대상에 대해 연구자 수동 검토를 완료하였다. 연구자 검토 결과는 O 13개, △ 6개, X 35개로 정리되었다. 이 검토는 전체 630개 응답의 평균 성능을 대체하는 지표가 아니라 G4 결과 중 논문 해석에 민감한 사례를 확인하기 위한 보완 검토이다. 최신 재생성 결과에서는 G4가 검색 성능은 개선했지만, 생성 응답 품질은 Qwen 기준 G3 4.01점에서 G4 3.99점으로 거의 유지되는 수준이었다. 따라서 응답 품질 결과는 G4의 주된 기여가 생성 품질 향상보다 검색 근거, 특히 이미지 검색 순위 개선에 있음을 보완적으로 보여준다.
...
4. 질문 기반 실습 단계 추정과 단계별 context map 기반 re-ranking을 통해 G3 대비 G4의 이미지 검색 성능을 개선하였다.
...
- G4는 질문 기반 실습 단계 추정과 context map을 이용해 이미지 후보의 순위를 개선하였다.
...
Qwen, Gemma, Llama 모델을 대상으로 응답 정확성, 구체성, 실습 단계 적합성, 안전성, 이해 용이성을 평가한다. 현재 rubric 기반 1차 응답 품질 평가와 G4 우선 검토 대상 54개에 대한 연구자 수동 검토가 완료되었다. 다만 전체 630개 응답을 모두 전문가가 재채점한 것은 아니므로, 최종 원고에서는 rubric 기반 평가와 연구자 보완 검토의 범위를 명확히 구분한다.
...
현재 상태에서 Method, Experimental Setup, Retrieval Results, Response Quality Evaluation 초안은 모두 작성 가능하다. Response Quality Evaluation에는 rubric 기반 1차 평가 결과와 G4 우선 검토 대상 54개에 대한 연구자 검토 결과를 함께 제시하되, 두 평가의 범위가 다르다는 점을 명확히 밝히는 것이 좋다.
...
5. rubric 기반 응답 품질 평가와 연구자 보완 검토 결과를 구분해 서술
...
| 응답 품질 연구자 검토 결과 | `SCIE용/31_researcher_review_checklist.md`, `SCIE용/excel/31_researcher_review_checklist.xlsx` |
