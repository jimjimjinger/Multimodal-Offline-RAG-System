...
본 연구는 협동 로봇 실습 매뉴얼을 대상으로, 학습자의 실습 질의에 대해 텍스트 설명과 이미지/도식 자료를 함께 검색하고 이를 기반으로 안내 응답을 생성하는 상황 인지형 멀티모달 RAG 구조를 제안한다.
...
- 제안 시스템은 협동 로봇 실습 교육에서 학습 보조 도구로 활용될 가능성이 있는가?
...
| G4 | Context-aware Multimodal RAG | 질문에서 실습 단계를 자동 추정하고, 해당 단계의 page 범위, section heading, keyword context map 기반 re-ranking을 추가한 구조 |

G2와 G3는 텍스트 검색 경로가 동일하므로 텍스트 성능은 동일하게 해석한다. G3의 핵심은 이미지/도식 검색 성능을 추가로 평가할 수 있다는 점이다. G4는 질문에서 실습 단계를 자동 분류한 뒤, 이미지 후보의 순위를 더 적절하게 조정하기 위해 실습 단계 정보를 반영한 구조이다.

## 5. 최종 검색 성능 결과

현재 논문용 기준 결과는 자동 단계 분류 기반 G4 결과를 기준으로 한다. 정답 실습 단계가 주어진 oracle-stage 결과는 실제 앱 사용 조건이 아니므로 본문 메인 비교에는 사용하지 않고, 필요한 경우 부록 또는 추가 분석에서 상한 성능으로만 언급한다. `09_stage_context_map` 및 `10_g4_results`는 초기 예비 실험 성격이므로 최종 논문 결과로 사용하지 않는다.
...
G3에서는 이미지 검색이 추가되었지만, Image Recall@1은 32.9%, Image MRR은 0.485로 낮게 나타났다. 즉, 관련 이미지를 Top-5 또는 Top-10 안에 포함시키는 것은 어느 정도 가능했지만, 정답 이미지를 상위 순위로 올리는 데 한계가 있었다.

G4에서는 질문에서 실습 단계를 자동 추정한 뒤, 해당 단계의 page 범위, section heading, keyword 정보를 기반으로 re-ranking을 수행하였다. 그 결과 G3 대비 Image Recall@5는 70.0%에서 75.7%로, Image Recall@10은 75.7%에서 87.1%로, Image MRR은 0.485에서 0.539로 개선되었다.

따라서 현재 결과는 다음과 같이 요약할 수 있다.

- 텍스트 검색은 이미 비교적 안정적이다.
- 멀티모달 RAG의 병목은 이미지 검색 순위와 텍스트-이미지 정렬 품질이다.
- G4의 상황 정보 기반 re-ranking은 정답 이미지를 더 상위 후보로 올리는 데 효과가 있었다.
- 다만 같은 page 또는 같은 section 안에 유사 이미지가 많은 경우에는 여전히 image-level distinction이 부족하다.

## 7. G4 설계 핵심

G4는 정답 이미지 파일명이나 정답 chunk ID를 직접 사용하지 않는다. 먼저 질문과 실습 단계 context profile 간 BGE-M3 의미 유사도를 비교하여 실습 단계를 자동 추정하고, 이후 실습 단계별로 다음 정보를 매핑한 context map을 사용한다.
...
- 질문에서 자동 추정된 실습 단계와 관련된 page 범위에 있는 텍스트/이미지 후보에 가중치를 부여한다.
- section heading과 keyword가 일치하는 후보의 점수를 보정한다.
- 기존 G3의 텍스트 검색, 이미지 검색, page proximity, 텍스트-이미지 매핑 점수는 유지한다.
- 자동 분류 신뢰도가 낮거나 후보가 애매하면 G4를 강제 적용하지 않고 G3 방식으로 fallback한다.
- 상황 정보는 후보를 새로 생성하는 것이 아니라, 기존 후보의 순위를 재조정하는 역할을 한다.

## 8. G4 개선 사례

G4의 개선 효과를 설명하기 위해 다음 세 가지 사례를 논문에 사용할 수 있다.

| 사례 | 질문 번호 | 실습 단계 | 정답 이미지 | G3 순위 | G4 순위 | 해석 |
|---|---|---|---|---:|---:|---|
| 티치 펜던트/USB 데이터 관리 | Q31 | 티치 펜던트/USB 데이터 관리 | `page_103_img_0_0.jpeg` | Top-10 밖 | 3위 | 자동 단계 분류가 맞았고 USB 관련 context가 반영되어 Top-5 안으로 상승 |
...
이 사례들은 G4가 단순히 전체 Recall만 높인 것이 아니라, 실제 실습 단계 맥락을 이용해 정답 이미지의 순위를 끌어올렸다는 근거로 사용할 수 있다.

## 9. G4 실패 사례와 한계

G4에서도 실패한 사례는 논문에서 한계 및 향후 연구로 정리할 수 있다.
...
| 시스템 관리/로그 | Q15 | 시스템 관리/로그 | `page_340_img_0_0.jpeg` | 자동 단계 분류가 UI/시스템 정보 확인으로 치우쳐 정답 이미지가 Top-10에 포함되지 않음 |

현재 G4는 page-level 또는 section-level context를 반영하는 데는 효과가 있으나, 같은 page 안의 여러 이미지 중 정확한 이미지를 구분하는 능력은 아직 부족하다. 이를 개선하려면 이미지 caption, bbox 주변 텍스트, 이미지 순서, 세부 단계 라벨을 추가로 활용해야 한다.
...
| 지표 | 의미 |
|---|---|
| Text Recall@k | Top-k 텍스트 후보 안에 정답 텍스트가 포함되는 비율 |
| Image Recall@k | Top-k 이미지 후보 안에 정답 이미지가 포함되는 비율 |
| Text MRR | 정답 텍스트가 높은 순위에 위치하는 정도 |
| Image MRR | 정답 이미지가 높은 순위에 위치하는 정도 |
| Both@k | Top-k 안에 정답 텍스트와 정답 이미지가 모두 포함되는 비율 |

텍스트 검색 평가는 파일럿 단계에서 relaxed 기준을 사용하였다. 즉, 정답 문장과 완전 일치하지 않더라도 같은 page, 핵심 keyword 포함, 의미적으로 유사한 답변이면 정답으로 인정하였다. 논문에서는 이 기준을 명확히 설명해야 하며, 가능하다면 strict 평가와 relaxed 평가를 분리해서 제시하는 방안도 고려한다.
...
각 항목은 1~5점으로 평가하고, 평균 점수를 기준으로 응답 품질을 비교한다. 현재 Qwen, Gemma, Llama를 대상으로 G2/G3/G4 조건의 630개 응답에 대한 자동 1차 평가를 완료하였다.
...
이 평가는 rubric 기반 자동 1차 평가이므로, 논문 최종본에서는 전문가 또는 연구자 수동 검토로 보완하는 것이 바람직하다. 최신 재생성 결과에서는 G4가 검색 성능은 개선했지만, 생성 응답 품질은 Qwen 기준 G3 4.01점에서 G4 3.99점으로 거의 유지되는 수준이었다.
...
4. 자동 실습 단계 분류와 단계별 context map 기반 re-ranking을 통해 G3 대비 G4의 이미지 검색 성능을 개선하였다.
5. 개선 사례와 실패 사례를 통해 상황 인지형 멀티모달 RAG의 효과와 한계를 함께 분석하였다.
...
- G4는 자동 실습 단계 분류와 context map을 이용해 이미지 후보의 순위를 개선하였다.
...
Qwen, Gemma, Llama 모델을 대상으로 응답 정확성, 구체성, 실습 단계 적합성, 안전성, 이해 용이성을 평가한다. 현재 자동 1차 응답 품질 평가까지 완료되었으며, 최종 원고에서는 수동 검토 또는 전문가 평가로 보완한다.
...
현재 상태에서 Method, Experimental Setup, Retrieval Results, Response Quality Evaluation 초안은 모두 작성 가능하다. 다만 Response Quality Evaluation은 자동 1차 평가 결과이므로, 최종 논문에서는 평가 성격을 명확히 밝히고 수동 검토 결과를 추가하는 것이 좋다.
...
논문 최종 결과로는 `11`, `15`, `16`, `17`, `22`, `29`, `30`번 산출물을 중심으로 사용하는 것이 적절하다. `12`번은 정답 실습 단계가 주어진 oracle-stage 참고 결과로만 사용한다.
