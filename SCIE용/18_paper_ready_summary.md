...
G4에서는 질문에서 실습 단계를 추정한 뒤, 해당 단계의 page 범위, section heading, keyword 정보를 기반으로 re-ranking을 수행하였다. 그 결과 G3 대비 Image Recall@5는 70.0%에서 75.7%로, Image Recall@10은 75.7%에서 87.1%로, Image MRR은 0.485에서 0.539로 개선되었다. 다만 Image Recall@1은 37.1%로 여전히 낮기 때문에, G4는 이미지 검색 문제를 해결한 방식이 아니라 관련 이미지 후보를 Top-5 또는 Top-10 안으로 더 잘 끌어올린 방식으로 해석한다.
...
이 평가는 rubric 기반 1차 평가이며, 이후 G4 결과 중 오류 및 애매 사례 중심의 54개 우선 검토 대상에 대해 연구자 수동 검토를 완료하였다. 연구자 검토 결과는 O 13개, △ 6개, X 35개로 정리되었다. 이 검토는 전체 630개 응답의 평균 성능을 대체하는 지표가 아니라 G4 결과 중 논문 해석에 민감한 사례를 확인하기 위한 보완 검토이다. 최신 재생성 결과에서는 G4가 검색 성능은 개선했지만, 생성 응답 품질은 Qwen 기준 G3 4.01점에서 G4 3.99점으로 거의 유지되는 수준이었다. 따라서 응답 품질 결과는 본 논문의 중심 성과가 아니라, 검색 성능 개선이 생성 품질 향상으로 항상 직접 연결되지는 않는다는 보조 분석으로 제시한다.
...
- 다만 Image Recall@1은 37.1%로 여전히 낮아, 이미지 검색 문제를 완전히 해결했다고 주장하지 않는다.
...
Qwen, Gemma, Llama 모델을 대상으로 응답 정확성, 구체성, 실습 단계 적합성, 안전성, 이해 용이성을 평가한다. 이 절은 중심 실험 결과가 아니라 로컬 LLM 기반 실습 안내의 가능성과 한계를 확인하는 보조 분석으로 배치한다. 현재 rubric 기반 1차 응답 품질 평가와 G4 우선 검토 대상 54개에 대한 연구자 수동 검토가 완료되었다. 다만 전체 630개 응답을 모두 전문가가 재채점한 것은 아니므로, 최종 원고에서는 rubric 기반 평가와 연구자 보완 검토의 범위를 명확히 구분한다.
