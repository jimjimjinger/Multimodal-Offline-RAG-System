...
| 단계 자동 분류기 | 질문과 실습 단계 context profile의 의미 유사도를 비교해 G4 적용 단계 추정 | `src/stage_classifier.py` |
...
| 평가 스크립트 | G1/G2/G3/G4 검색 성능, 단계 자동 분류, 응답 품질 평가 | `scripts/evaluate_scie_all_groups.py`, `scripts/evaluate_stage_classifier.py`, `scripts/evaluate_scie_g4_auto_retrieval.py`, `scripts/evaluate_response_quality.py` |
...
2. G4 조건에서는 질문과 실습 단계 context profile을 비교해 실습 단계를 자동 추정한다.
3. 자동 분류 신뢰도가 낮거나 1위와 2위 후보가 애매하면 G4를 적용하지 않고 G3 방식으로 검색한다.
...
- G4는 자동 단계 분류 오차가 발생할 수 있으므로, 낮은 신뢰도 질문은 G3로 fallback하도록 구성했다.
...
- 응답 품질 평가는 자동 1차 평가까지 수행했으며, 최종 논문에서는 수동 검토 또는 전문가 평가로 보완하는 것이 바람직하다.
- 8GB 이하 RAM 환경을 목표로 하지만, 현재 전체 정량 평가는 개발용 노트북 환경에서 수행되었다.
