...
from paths import VECTOR_DB_DIR, configure_model_cache, resolve_image_path

# 1. 페이지 레이아웃 및 리소스 로드
st.set_page_config(layout="wide", page_title="Doosan Robotics Smart RAG (Gemma)")
...
# --- Gemma 2.5 2B 답변 생성 함수 (Ollama API) ---
def generate_answer(question, context):
    ...
    try:
        ...
        return "⏳ Gemma 2.5:2b가 답변을 생성하는 데 시간이 너무 오래 걸립니다."
...
with st.sidebar:
    ...
    st.info("BGE-M3 + ChromaDB + Gemma 2.5 2B")
    st.write("✅ **LLM:** gemma2:2b")
...
if query:
    ...
    if results['documents'] and len(results['documents'][0]) > 0:
        ...
        # Top-5 문서를 랭킹 순으로 하나의 종합 컨텍스트로 병합
        combined_context = ""
        ...
        for rank, meta in enumerate(retrieved_metas):
            ...
            for img_p in paths:
                img_path = resolve_image_path(img_p)
                img_key = str(img_path)
                if img_key not in seen_paths and img_path.exists():
        ...
        with st.spinner('Gemma 2.5:2b가 Top-5 매뉴얼 문서를 종합 분석 중입니다...'):
